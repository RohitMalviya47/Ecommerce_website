...
const Navber = () => {
  ...
  return (
    ...
    <Link to='/'> <img src={logo} className="w-36 rounded" alt=""  /></Link> 

      <ul className="hidden sm:flex gap-5 text-sm  text-black">
        <NavLink to="/" className="flex flex-col items-center gap-1 ">
          <p>HOME</p>
          <hr className="w-2/4 border-none h-[1.5px] bg-gray-700 hidden text-black" />
        </NavLink>
    ...
    <img onClick={()=>setShowSearch(true)} src={assets.search_icon} className="w-5 cursor-pointer" alt="" />
      ...
      <div
        className={`absolute right-0 top-0 bottom-0 overflow-hidden transition-all bg-white ${
          visible ? "w-full" : "w-0"
        }`}
      >
